...
handle_info({#'basic.deliver'{}, {amqp_msg, #'P_basic'{}, Content}}, State = #{tid := Tid}) ->
	...
	Last = ets:update_counter(Tid, classic_last, {2, 1}, {classic_last, 0}),
	case Seq =:= Last of
		true -> ok;
		_ ->
			logger:error("consumer (classic) compare 'seq' resulted in error - seq:~p classic_last:~p diff:~p",
				[Seq, Last, Last - Seq]),
			true = ets:insert(Tid, {classic_last, Seq})
	end,
	if Seq rem 1000 =:= 0 -> logger:info("consumer (classic) reached seq:~p", [Seq]); true -> ok end,
	{noreply, State};
...
handle_info(connect, OldState = #{tid := Tid}) ->
	...
	logger:info("consumer (classic) (re)connected seq:~p~n", [ets:lookup(Tid, classic_last)]),
	{noreply, State};
handle_info(Msg, State) ->
	logger:warning("consumer (classic) handle unexpected msg:~p", [Msg]),
	{noreply, State}.

terminate(Reason, _State) ->
	logger:warning("consumer (classic) terminated by reason:~p", [Reason]),
	timer:sleep(3000),
	ok.
...
%% @private
connect(State) ->
	ConnProps = [{<<"connection_name">>, longstr, <<"classic_consumer">>}],
	...
	Queue = <<"rb_classic_queue_", (atom_to_binary(node()))/binary>>,
